...
class xssRecon:
    ...
    def logo(self):
        print(Fore.GREEN + """
 __  ______ ____   ____ ____      ___        ___     _____ ____  
\ \/ / ___/ ___| / ___|  _ \    / \ \      / / |   | ____|  _ \ 
 \  /\___ \___ \| |   | |_) |  / _ \ \ /\ / /| |   |  _| | |_) |
 /  \ ___) |__) | |___|  _ <  / ___ \ V  V / | |___| |___|  _ < 
/_/\_\____/____/ \____|_| \_\/_/   \_\_/\_/  |_____|_____|_| \_\

    XSSCrawler - Automated refl. XSS Scanner
    Shreyash Khare
...
""" + Style.RESET_ALL)
    ...
    def spawn_browser(self, visibility):
        ...
        self.options.add_argument("--no-sandbox")  # Add this option to avoid Chrome error
        ...
        for link in self.usable_links:
            if "=" in link:
                ...
                for payload in self.payloads:
                    ...
                    self.single_xss_check(url=str(exploit_url), payload=payload,
                                          parameter=(full_link.split("=")[int(equal_counter) - 1]))
    ...
    def scan_one_url(self, url):
        ...
        for payload in self.payloads:
            self.single_xss_check(str(url) + str(payload), payload=payload,
                                  parameter=url.split("=")[int(equal_counter) - 1])
        if not self.vulns:
            print(Fore.YELLOW + "[-] No vulnerabilities found")
            self.quit_driver()
            sys.exit()
...
""".format(parameter + "=", payload, self.counter))
    ...
    def quit_driver(self):
        if self.driver is not None:
            self.driver.quit()  # Quit driver if it's initialized
